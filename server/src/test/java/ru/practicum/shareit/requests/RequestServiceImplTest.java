...
class RequestServiceImplTest {
    ...
    public void getAllItemRequestsWithResponsesCurrentUser() {
        ...
        assertAll(
                ...
                () -> assertEquals(
                        serviceResult.get(0).getItems().size(), 0, "Ответов на запрос не должно быть"
                ),
                () -> assertEquals(
                ...
                () -> assertEquals(
                        serviceResult.get(1).getItems().size(), 1, "Должен быть один ответ для запроса"
                ),
                () -> assertEquals(
                        serviceResult.get(1).getItems().get(0).getId(),
                        item3.getId(),
                ...
                () -> assertEquals(
                        serviceResult.get(2).getItems().size(), 2, "Должно быть два ответа на запрос"
                ),
                () -> assertEquals(
                        serviceResult.get(2).getItems().get(0).getId(),
                        item1.getId(),
                        String.format("Ответом на запрос должен быть %s", item1)
                ),
                () -> assertEquals(
                        serviceResult.get(2).getItems().get(1).getId(),
                        item2.getId(),
    ...
    public void getAllItemRequests() {
        ...
        List<ItemRequestWithResponsesDto> serviceResult = requestService.getAllItemRequests(0, 3, userId);
        ...
        List<ItemRequestWithResponsesDto> serviceResultSecondTest = requestService.getAllItemRequests(1, 2, userId);
    ...
    public void getItemRequestWithResponsesById() {
        ...
        ItemRequestWithResponsesDto itemRequestWithResponsesDto = requestService
                .getItemRequestWithResponsesById(itemRequestDto1.getId(), userId);

        assertAll(
                () -> assertEquals(itemRequestWithResponsesDto.getItems().size(), 2),
                () -> assertEquals(
                        itemRequestWithResponsesDto.getItems().get(0).getId(),
                        idForItem1,
                        String.format("id для первого ответа должен быть %s", idForItem1)
                ),
                () -> assertEquals(
                        itemRequestWithResponsesDto.getItems().get(1).getId(),
                        idForItem2,
